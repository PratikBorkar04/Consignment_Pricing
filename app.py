...
def predict():
    if request.method == 'POST':
        ...
        country_dict_reversed = {
            'Congo': 0, 'Côte d\'Ivoire': 1, 'Haiti': 2, 'Mozambique': 3, 'Nigeria': 4,
            'Rwanda': 5, 'South Africa': 6, 'Tanzania': 7, 'Uganda': 8, 'Vietnam': 9,
            'Zambia': 10, 'Zimbabwe': 11
        }
        country_name = country_dict_reversed[request.form['country_name']]

        vendor_dict_reversed = {
            'CIP': 0, 'DAP': 1, 'DDP': 2, 'EXW': 3, 'FCA': 4, 'RDC': 5
        }
        vendor_name = vendor_dict_reversed[request.form['vendor_name']]

        shipment_mode_dict_reversed = {
        ...
        manufacturing_site_dict_reversed = {
            'China': 0, 'Cyprus': 1, 'Nashik': 2, 'Port Elizabeth': 3, 'Thailand': 4,
            'Switzerland': 5, 'Haarlem': 6, 'France': 7, 'Germany': 8, 'India': 9,
            'Latina': 10, 'Poland': 11, 'Canada': 12, 'South Africa': 13,
            'Puerto Rico': 14
        }
        ...
        input_data = [[country_name, vendor_name, shipment_mode, unit_of_measure, line_item_quantity,
                       line_item_value, pack_price, unit_price, weight, manufacturing_site, distance]]
        prediction = model.predict(input_data)[0]

        # Format the prediction result as a string
        result = f"The predicted consignment price is: {prediction:.2f}"
